...
func runVolListCommand(ctx context.Context, rep repo.Repository) error {
	...
	if err == nil && len(snaps) > 0 {
		...
		fmt.Printf("" +
			"      Timestamp                   Repository ID              #Blocks      #Dirs   CLen   #CBlocks      #CDirs         WBC       VolID / SnapID\n" +
			"----------------------- --------------------------------- ------------ ---------- ---- ------------ ---------- ---------------- ---------------\n")

		for _, s := range snaps {
			fmt.Printf("%s %s %12d %10d %4d %12d %10d %16d %s / %s\n",
				formatTimestamp(s.Manifest.StartTime), s.Manifest.RootObjectID(), s.NumBlocks, s.NumDirs,
				s.ChainLength, s.ChainedNumBlocks, s.ChainedNumDirs, s.WeightedBlockCount,
