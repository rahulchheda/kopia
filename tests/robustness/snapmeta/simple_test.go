...
func TestSimpleWithIndex(t *testing.T) {
	...
	type storeOp struct {
		...
		key   string
		value []byte
		idxOp map[string]robustness.IndexOperation
	}
			...
			storeOperations: []storeOp{
				{
					...
					value: []byte("some-value"),
					idxOp: map[string]robustness.IndexOperation{
						"some-index-key": robustness.AddToIndexOperation,
					},
				},
				{
					op:    storeOpType,
			...
			storeOperations: []storeOp{
				{
					...
					idxOp: map[string]robustness.IndexOperation{
						"some-index-key":      robustness.AddToIndexOperation,
						"some-more-index-key": robustness.AddToIndexOperation,
					},
				},
				{
					op:    storeOpType,
					...
					value: []byte("some-different-value"),
					idxOp: map[string]robustness.IndexOperation{
						"some-index-key": robustness.RemoveFromIndexOperation,
					},
				},
			},
			expectedKVs: map[string]expResult{
