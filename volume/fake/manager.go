// Package fake emulates a volume manager.
package fake
...
// package errors.
var (
	ErrInvalidProfile = fmt.Errorf("invalid GetBlockReaderArgs.Profile: expected a file path or a ReaderProfile")
)
...
func (p *manager) GetBlockReader(args volume.GetBlockReaderArgs) (volume.BlockReader, error) {
	...
	case string:
		...
		data, err := ioutil.ReadFile(filepath.Clean(x))
		if err == nil {
...
func (rp *ReaderProfile) Validate() error {
	...
	for i, r := range rp.Ranges {
		if ea > r.Start {
			return fmt.Errorf("range[%d] start overlaps previous range", i) // nolint
		}

		if r.Count == 0 {
			return fmt.Errorf("range[%d] count is 0", i) // nolint
		}
...
func (rp *ReaderProfile) GetBlock(ctx context.Context, blockAddr int64) (io.ReadCloser, error) {
	...
	for i, r := range rp.Ranges {
		...
		if r.template == nil {
			...
			if err != nil {
				return nil, fmt.Errorf("invalid template in range %#v", r) // nolint
			}
	...
	if rangeTemplate != nil { // nil if out of range
		_ = rangeTemplate.Execute(&buf, bta)
	}

	if buf.Len() > 0 {
		copies := fillBufSz / buf.Len()
		fill := bytes.Repeat(buf.Bytes(), copies+1)
		fill = fill[:256]
		nFill := int(rp.blockSize) / fillBufSz
